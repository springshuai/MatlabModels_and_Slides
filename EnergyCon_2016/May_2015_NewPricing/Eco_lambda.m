function [ Save, opt_K_U, opt_K_L, opt_K_R ] = Eco_lambda( A_d )
...
% find the upper- and lower- bounds of optmimal k
        for k = 1:1:K
            if k<=c
                ...
                Marginal_gain_U(k) = lambda/(mu+theta*P_star)*(Block(ca,k) - Block(ca,k+1))*P_star*(g-v_0/(1-epsilon*(P_m/k)));
                Marginal_gain_L(k) = lambda/(mu+theta*P_m/k)*(Block(ca,k) - Block(ca,k+1))*(P_m/k)*(g-v);
            end
...
% find the optimal k between the upper- and lower- bounds         
        if opt_K_U(ca,idx)~=opt_K_L(ca,idx)
            ...
            for i = opt_K_L(ca,idx):1:opt_K_U(ca,idx);% 'i' is the candidate of the optimal 'k'
                ...
                for j = 1:1:i+1
                    RealCost(i-opt_K_L(ca,idx)+1,1) = RealCost(i-opt_K_L(ca,idx)+1,1)+Pr_Disch(j)*Cost( j-1, c, g, v_0, P_m, epsilon);
                end
...
% find the upper- and lower- bounds of optmimal k
        for k = 1:1:K
            ...
            if k<=c
                ...
                Marginal_gain_U(k) = lambda/(mu+theta*P_star)*(Block(ca,k) - Block(ca,k+1))*P_star*(g-v_0/(1-epsilon*(P_m/k)))- workload;
                Marginal_gain_L(k) = lambda/(mu+theta*P_m/k)*(Block(ca,k) - Block(ca,k+1))*(P_m/k)*(g-v)- workload;
            end
...
cap = ['\caption{ Saving as coming rate increases ( $\mu=$' num2str(mu) ' ,$\theta=$' num2str(theta) ')}'];
string_parameters=['$\lambda$ goes from ' num2str(lambda_min) ' to ' num2str(lambda_max) ',$\mu=$' num2str(mu) ',$\theta = $' num2str(theta)];
axis_options = 'legend entries={Saving in Scheme 1,Saving in Scheme 2, Saving by using battery}';
