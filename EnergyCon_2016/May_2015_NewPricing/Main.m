...
axis_options = 'legend entries={Scheme 1,Scheme 2 without unplugging cost,Scheme 2 with unplugging cost,Battery only,Grid only}';
x_label = 'Number of spots $k$';
y_label = 'Cost per hour';
filename = 'Variation_K.tex';
fid=fopen(['Data_in_TEX/' filename],'w');
c=clock;
infos=['%Obtained from funtion Main with parameters of' string_parameters ', run on ' int2str(c(3)) '/' int2str(c(2)) '/' int2str(c(1)) ' at ' int2str(c(4)) ':' int2str(c(5)) ':' int2str(c(6))];
fprintf(fid,'%s\n',infos);
