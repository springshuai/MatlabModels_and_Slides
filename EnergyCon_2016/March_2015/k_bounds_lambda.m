function [ k_U_1, k_L_1, k_U_2, k_L_2 ] = k_bounds_lambda()
...
function [sn] = s_L (n)
    if n<=c_pat
        sn = p_star_pat*(g-p_EV_pat(p_star_pat))/(mu+Tht_fix*p_star_pat);
    else
        sn = P_m*(p_EV_pat(P_m/(n-1))-p_EV_pat(P_m/n))/(mu+Tht_fix*p_star_pat);
    end
end
...
function [sn] = s_U (n)
    if n<=c_pat
        ...
        sn = p_star_pat*(g-p_EV_pat(p_star_pat))/(mu+Tht_fix*(P_m/n));
    end
end

k_U_1 = zeros (1,nb_Lambda);
k_L_1 = zeros (1,nb_Lambda);
k_U_2 = zeros (1,nb_Lambda);
k_L_2 = zeros (1,nb_Lambda);
...
%         % look for the upper and lower bounds 
        ...
        if l*(Block_1_pre-Block_1)*s_U(k)>=A_d
            k_U_1(1,i)=k;
        else
        end
        
        if l*(Block_1_pre-Block_1)*s_L(k)>=A_d
            k_L_1(1,i)=k;
        else
        end 
        % this bound here does NOT consider the worklaod of removing 
        if l*(Block_2_pre-Block_2)*s_U(k)>=A_d
            k_U_2(1,i)=k;
        else
        end 
        % this bound here does not consider the workload of removing 
        if l*(Block_2_pre-Block_2)*s_L(k)>=A_d
            k_L_2(1,i)=k;
        else
        end       
%         % upper bound for the first scheme
...
% parameters = [' $\chi$ = 0.85; $g$ = 0.5; $U$ = 0.2; $A_d$ = 0.02; $v_0$ = 0.1; $\alpha$ = 0.02; $\mu$ = 1; $\theta$ = 0.1; P = 10; $P_m$ = 200; '];
 axis_options = 'legend entries={$k^U_1,k^L_1,k^U_2,k^L_2}';
 x_label = 'Number of EVs coming per hour';
 y_label = 'Bounds of optimal number of discharger';
 filename = 'k_bounds_lambda.tex';
...
% fprintf(fid_lambda,'%s\n',parameters);
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_Lambda
     fprintf(fid,'%s','(',num2str(L_vec(i)),',',num2str(k_U_1(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_Lambda
     fprintf(fid,'%s','(',num2str(L_vec(i)),',',num2str(k_L_1(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_Lambda
     fprintf(fid,'%s','(',num2str(L_vec(i)),',',num2str(k_U_2(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
