function [Pi] = Steady_state_distribution( K, lambda, mu, theta, P, P_m )
...
for m = 0:1:K
    for n = m:1:K
        ...
        if m > 0
            if C - m > 0
                TM (S(m,n),S(m-1,n))= theta*m*P;  % a discharging car depleted when discharging full rate
            else
                TM (S(m,n),S(m-1,n))= theta*P_m;    % a discharging car depleted when discharging partial rate
            end
        end
