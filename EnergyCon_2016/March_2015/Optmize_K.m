function [ Save, opt_K_U, opt_K_L, opt_K_R ] = Optmize_K( mu, theta, alpha, P_m, A_d, g, v_0 )
...
% find the upper- and lower- bounds of optmimal k
        for k = 1:1:K
            if k<=c
                ...
                Marginal_gain_U(k) = lambda/(mu+theta*P_m/k)*(Block(ca,k) - Block(ca,k+1))*P_star*(g-v);
                Marginal_gain_L(k) = lambda/(mu+theta*P_star)*(Block(ca,k) - Block(ca,k+1))*(alpha*P_m^2*(1/(k-1) - 1/k));
            end

            if A_d <= Marginal_gain_U(k)
               opt_K_U(ca,lambda-L_min+1) = k;
            end
            if A_d <= Marginal_gain_L(k)
               opt_K_L(ca,lambda-L_min+1) = k;
               Gain_max(ca,1) = Gain_max(ca,1) + (Marginal_gain_L(k)-A_d);
            else
            end
        end
% find the optimal k between the upper- and lower- bounds         
        if opt_K_U(ca,lambda-L_min+1)~=opt_K_L(ca,lambda-L_min+1)
            opt_K_U_L_gap = opt_K_U(ca,lambda-L_min+1)-opt_K_L(ca,lambda-L_min+1)+1;
            RealCost = zeros(opt_K_U_L_gap,1);
            RealCost_min = P_m*g;
            for i = opt_K_L(ca,lambda-L_min+1):1:opt_K_U(ca,lambda-L_min+1);% 'i' is the candidate of the optimal 'k'
                Pr_Disch = Steady_state_distribution( i, lambda, mu, theta, P_star, P_m );
                for j = 1:1:i+1
                    RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1) = RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1)+Pr_Disch(j)*Cost( j-1, c, g, v_0, P_m, alpha);
                end
                RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1)=RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1)+i*A_d;
                if RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1)<RealCost_min
                    RealCost_min = RealCost(i-opt_K_L(ca,lambda-L_min+1)+1,1);
                    opt_K_R(ca,lambda-L_min+1) = i;
                else
                end
            end
        else
            opt_K_R(ca,lambda-L_min+1)=opt_K_L(ca,lambda-L_min+1);
        end

        if opt_K_R(ca,lambda-L_min+1)<=c
            Save (ca,lambda-L_min+1) = Gain_max(ca,1)/(P_m*g);
        else
            Save (ca,lambda-L_min+1)= RealCost_min/(P_m*g);
            Save (ca,lambda-L_min+1)=1-Save (ca,lambda-L_min+1);
        end
...
% find the optimal k between the upper- and lower- bounds        
        if opt_K_U(ca,lambda-L_min+1)~=opt_K_L(ca,lambda-L_min+1)
            ...
            opt_K_R(ca,lambda-L_min+1)=opt_K_U(ca,lambda-L_min+1);
        end
        
        if opt_K_R(ca,lambda-L_min+1)<=c
            Save (ca,lambda-L_min+1) = Gain_max(3,1)/(P_m*g);
        else
            Save (ca,lambda-L_min+1)= RealCost_min/(P_m*g);
            Save (ca,lambda-L_min+1)=1-Save (ca,lambda-L_min+1);
        end
end
cap=['\caption{Gap between the upper-bound and lower-bound of $k$ and its real value, when $\mu=$' num2str(mu) ', $\theta=$' num2str(theta) '}'];
% string_parameters=['$\mu=$' num2str(mu) ',$\theta=$' num2str(theta)];
axis_options = 'legend entries={Upper-bound,Optimum,Lower bound}';
x_label = 'Number of EVs coming per hour';
y_label = 'Number of dischargers';
filename = 'opt_K.tex';
fid=fopen(['Data_in_TEX/' filename],'w');
c=clock;
infos=['%Obtained from funtion Optmize_K with parameters of' string_parameters ', run on ' int2str(c(3)) '/' int2str(c(2)) '/' int2str(c(1)) ' at ' int2str(c(4)) ':' int2str(c(5)) ':' int2str(c(6))];
fprintf(fid,'%s\n',infos);
...
fprintf(fid,'%s\n','\addplot coordinates{');
for i = 1:1:size(opt_K_U,2)
    fprintf(fid,'%s','(',num2str(i+9),',',num2str(opt_K_U(1,i)), ')');
end
fprintf(fid, '%s\n', '};');

fprintf(fid,'%s\n','\addplot coordinates{');
for i = 1:1:size(opt_K_R,2)
    fprintf(fid,'%s','(',num2str(i+9),',',num2str(opt_K_R(1,i)), ')');
end
fprintf(fid, '%s\n', '};');

fprintf(fid,'%s\n','\addplot coordinates{');
for i = 1:1:size(opt_K_L,2)
    fprintf(fid,'%s','(',num2str(i+9),',',num2str(opt_K_L(1,i)), ')');
end
fprintf(fid, '%s\n', '};');
