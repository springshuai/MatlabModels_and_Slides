function [ Save, opt_K_U, opt_K_L, opt_K_R ] = Eco_theta( mu, alpha, P_m, A_d, g, v_0 )
...
theta_max = 0.25;
theta_min = 0.05;
theta_vec = linspace(theta_min, theta_max, 21);
nb_theta = size(theta_vec,2);
Save = zeros(3,nb_theta); % saving for nb_lambda values of lambda, in three cases
opt_K_U = zeros(3,nb_theta);% the upper-bound of optimal number of spots for nb_lambda values of lambda, in three cases
opt_K_L = zeros(3,nb_theta);% the upper-bound of optimal number of spots for nb_lambda values of lambda, in three cases
opt_K_R = zeros(3,nb_theta);% the real optimal number of spots for nb_lambda values of lambda, in three cases


for idx = 1:1:nb_theta
    theta = theta_vec(idx);
...
% find the optimal k between the upper- and lower- bounds         
        ...
        if opt_K_R(ca,idx)<=c
            Save (ca,idx) = Gain_max(ca,1)/(P_m*g);
        else
            Save (ca,idx)= RealCost_min/(P_m*g);
...
% find the optimal k between the upper- and lower- bounds        
        ...
        if opt_K_R(ca,idx)<=c
            Save (ca,idx) = Gain_max(3,1)/(P_m*g);
        else
            Save (ca,idx)= RealCost_min/(P_m*g);
            Save (ca,idx)=1-Save (ca,idx);
        end
end
cap = ['\caption{Cost saved as surplus energy increases, where $\theta$ goes from ' num2str(theta_min) ' to ' num2str(theta_max) ', $\mu=$' num2str(mu) ', $\lambda=$' num2str(lambda) ' }'];
%string_parameters=['$\theta$ goes from ' num2str(theta_min) ' to ' num2str(theta_max) ',$\mu=$' num2str(mu) ',$\lambda=$' num2str(lambda)];
axis_options = 'legend entries={Saving in Scheme 1,Saving in Scheme 2}';
x_label = 'Average surplus energy left (kWh)';
y_label = 'Saving per hour in \%';
filename = 'Eco_theta.tex';
...
fprintf(fid,'%s\n','\addplot coordinates{');
for i = 1:1:nb_theta
    fprintf(fid,'%s','(',num2str(1/theta_vec(i)),',',num2str(Save(1,i)), ')');
end
fprintf(fid, '%s\n', '};');

fprintf(fid,'%s\n','\addplot coordinates{');
for i = 1:1:nb_theta
    fprintf(fid,'%s','(',num2str(1/theta_vec(i)),',',num2str(Save(3,i)), ')');
end
fprintf(fid, '%s\n', '};');
