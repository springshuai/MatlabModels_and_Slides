function [ Save_vec_pat_1, Save_vec_pat_2, OptK_vec_1, OptK_vec_2 ] = Exhaustive_k_theta()
...
%        Park_vec = MMCK_classic( k, L_fix, t, p_star_pat, P_m );
        ...
        d_vec = Steady_state_distribution( k, L_fix, mu, t, p_star_pat, P_m );
        d_prime_vec = MMCK_pseudo( k, L_fix, mu, t, p_star_pat, P_m );
        for j = 1:1:k+1
...
% parameters = [' $\chi$ = 0.85; $g$ = 0.5; $U$ = 0.2; $A_d$ = 0.02; $v_0$ = 0.1; $\alpha$ = 0.02; $\mu$ = 1; $\theta$ = 0.1; P = 10; $P_m$ = 200; '];
 axis_options_theta = 'legend entries={Saving in Scheme 1,Saving in Scheme 2}';
 x_label_theta = 'Number of EVs coming per hour';
 y_label_theta = 'Saving in \%';
 filename_theta = 'Eco_theta.tex';
 fid_theta=fopen(['Data_in_TEX/' filename_theta],'w');
%  c=clock;
...
% fprintf(fid_lambda,'%s\n',parameters);
 ...
 fprintf(fid_theta,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid_theta,'%s','(',num2str(Tht_vec(i)),',',num2str(Save_vec_pat_1(1,i)), ')');
 end
 fprintf(fid_theta, '%s\n', '};');
 
 fprintf(fid_theta,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid_theta,'%s','(',num2str(Tht_vec(i)),',',num2str(Save_vec_pat_2(1,i)), ')');
 end
 fprintf(fid_theta, '%s\n', '};');
  
 fprintf(fid_theta,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid_theta,'%s','(',num2str(Tht_vec(i)),',',num2str(OptK_vec_1(1,i)), ')');
 end
 fprintf(fid_theta, '%s\n', '};');
 
 fprintf(fid_theta,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid_theta,'%s','(',num2str(Tht_vec(i)),',',num2str(OptK_vec_2(1,i)), ')');
 end
 fprintf(fid_theta, '%s\n', '};');
% fprintf(fid_lambda, '%s\n', '};');
% 
