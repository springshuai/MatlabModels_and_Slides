function [ Cost_matrix_1, Cost_matrix_2, Cost_matrix_3 ] = Main_K()
...
%%%%%%%%%%%%%%%%%%%%% print the TEX files %%%%%%%%%%%%%%%%%%%%%%%%%
%
 axis_options_lambda = 'legend entries={Cost without purchase from EVs,Cost without unplugging procedure,Cost after carring out unplugging}';
 x_label_lambda = 'Number of dischargers';
 y_label_lambda = 'Cost per hour';
 filename = 'Cost_K.tex';
 fid=fopen(['Data_in_TEX/' filename],'w');
 time =clock;
 infos = ['% Obtained from Main_K(), run on ', int2str(time(3)), '/', int2str(time(2)), '/',int2str(time(1)), ' at ', int2str(time(4)), ':', int2str(time(5)), ':', int2str(time(6)), 'when $\theta$ = ', num2str(Tht_fix), '$\lambda$= ', num2str(L_max)]; 
 fprintf(fid,'%s\n',infos);
 fprintf(fid,'%s\n',['{\footnotesize\begin{tikzpicture}\begin{axis}[legend style={at={(1,0.35)},anchor=south east},width=\figwidth,height=\figheight,cycle list name=\mylist,every axis legend/.append style={nodes={right}},xlabel=' x_label_lambda ',ylabel=' y_label_lambda ',' axis_options_lambda ']']);
 
 fprintf(fid, '%s\n', '% Cost without purchase from EVs');
 fprintf(fid,'%s\n','\addplot coordinates{');
 for k = 1:1:K_max
     fprintf(fid,'%s','(',num2str(k),',',num2str(P_m*g), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for k = 1:1:K_max
     fprintf(fid,'%s','(',num2str(k),',',num2str(Cost_matrix_1(1,k)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for k = 1:1:K_max
     fprintf(fid,'%s','(',num2str(k),',',num2str(Cost_matrix_2(1,k)), ')');
 end
 fprintf(fid, '%s\n', '};');
 
 fprintf(fid, '%s\n','\end{axis}\end{tikzpicture}}');

 Parameters = ['% Other parameters are: $\eta$=', num2str(eta),';$\epsilon$=',...
 num2str(epsilon),';$g$=', num2str(g),';Unplug=', num2str(Unplug),';$A_d$=',...
 num2str(A_d),';$v_0$=', num2str(v_0), ';$\mu$=', num2str(mu), ';$P_m$=', num2str(P_m)];
