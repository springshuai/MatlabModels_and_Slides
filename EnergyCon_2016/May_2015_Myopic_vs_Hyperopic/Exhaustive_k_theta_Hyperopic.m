function [ Save_vec_H_1, Save_vec_H_2, Save_vec_H_3, OptK_vec_H_1, OptK_vec_H_2, OptK_vec_H_3, Cost_matrix_1, Cost_matrix_2, Cost_matrix_3 ] = Exhaustive_k_theta_Hyperopic()
...
%%%%%%%%%%%%%%%
c_vec_H = zeros(1,nb_theta);
Cost_matrix_1 = zeros(nb_theta, K_max);
Cost_matrix_2 = zeros(nb_theta, K_max);
Cost_matrix_3 = zeros(nb_theta, K_max); % % without unplugging cost
...
Cost_vec_3 = zeros(2,nb_theta); % minimized from Cost_matrix_3, for each lambda
...
%        Park_vec = MMCK_classic( k, L_fix, t, p_star_pat, P_m );
        [ P0_vec, Cost_ins, c_vec_H(i) ] = P0_vec_Hyperopic( mu, t , k, g, v_0, epsilon, P_m );
...
%
 ...
 time =clock;
 infos = ['% Obtained from Exhaustive_k_theta_Hyperopic(), run on ', int2str(time(3)), '/', int2str(time(2)), '/',int2str(time(1)), ' at ', int2str(time(4)), ':', int2str(time(5)), ':', int2str(time(6)), 'when $\lambda$ = ', num2str(L_fix)]; 
 fprintf(fid,'%s\n',infos);

 fprintf(fid, '%s\n', '% Saving in Scheme 1');
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(Save_vec_H_1(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(Save_vec_H_2(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(Save_vec_H_3(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(OptK_vec_H_1(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(OptK_vec_H_2(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
 ...
 fprintf(fid,'%s\n','\addplot coordinates{');
 for i = 1:1:nb_theta
     fprintf(fid,'%s','(',num2str(1/Tht_vec(i)),',',num2str(OptK_vec_H_3(1,i)), ')');
 end
 fprintf(fid, '%s\n', '};');
